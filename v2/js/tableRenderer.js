...
function displayTable(data, headers) {
    ...
    data.forEach((row, rowIndex) => {
        ...
        // Store unique identifier for the row (Service0 value)
        tr.setAttribute('data-service-id', row['Service0'] || '');
        ...
        headers.forEach(header => {
            ...
            // Check if this is a Jenkins Link, build column, or Last Committer and contains HTML
            if ((header === 'Jenkins Link' || header === 'build_images' || header === 'build_bundle' || header === 'Last Committer') && cellValue.includes('<a href=')) {
                ...
                if (cellValue.toLowerCase() === 'not found') {
                    ...
                    if (service) {
                        const url = `https://backoffice.${env}.catonet.works/applications?q=${encodeURIComponent(service)}`;
            ...
            td.setAttribute('data-service-id', row['Service0'] || '');
            tr.appendChild(td);
        });
...
export function getFilteredData() {
        ...
        dataToShow = dataToShow.filter(row => {
            ...
            Object.keys(row).forEach(key => {
                if (visibleHeaders.includes(key) && isVersionColumn(key, versionsFileNames)) {
                    const versionValue = (row[key] || '').toString().trim();
                    // Skip empty, "Not found", "Error", and "Build not found" values
                    if (versionValue) {
            ...
            // Only show rows that have at least one valid version AND that version is unequal
    ...
    if (showUnequalBundlesOnly) {
        dataToShow = dataToShow.filter(row => {
            ...
            let hasValidBundle = false; // Track if we found at least one valid bundle value
            
            Object.keys(row).forEach(key => {
                if (visibleHeaders.includes(key) && isBundleColumn(key)) {
                    const bundleValue = (row[key] || '').toString().trim();
                    // Skip empty, "Not found", "Error", and "Build not found" values
                    if (bundleValue && 
                        bundleValue.toLowerCase() !== 'not found' && 
                        bundleValue.toLowerCase() !== 'error' &&
                        bundleValue.toLowerCase() !== 'build not found') {
                        hasValidBundle = true; // Found a valid bundle value
            ...
            // Only show rows that have at least one valid bundle AND that bundle is unequal
            return hasValidBundle && hasUnequalBundle;
        });
    }
